      ...
      <div className="w-full max-w-lg">
            ...
            <React.Fragment key={s}>
              <div className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium transition-all ${
                  ...
                  i === step ? "bg-emerald-600 text-white ring-2 ring-emerald-600/20 ring-offset-2 ring-offset-background" :
              ...
              {i < STEPS.length - 1 && <div className={`h-px flex-1 ${i < step ? "bg-emerald-600" : "bg-border"}`} />}
            </React.Fragment>
          ))}
        </div>
              ...
              <div className="space-y-2 mb-6">
                {[
                  ...
                  <label key={m.value} className={`flex items-start gap-3 p-3.5 rounded-lg border cursor-pointer transition-colors ${
                    form.aiMode === m.value ? "border-emerald-600/50 bg-emerald-600/5" : "border-border hover:border-white/10"
                  }`}>
                    <input
                      type="radio"
                      name="mode"
                      ...
                      className="mt-1 accent-emerald-600 sr-only"
                    />
                    <div className={`w-4 h-4 rounded-full border-2 mt-0.5 flex items-center justify-center shrink-0 ${
                      form.aiMode === m.value ? "border-emerald-600 bg-emerald-600" : "border-border"
                    }`}>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {m.recommended && <span className="text-[10px] bg-emerald-600/15 text-emerald-400 border border-emerald-600/20 rounded px-1.5 py-0.5">Recommandé</span>}
                      </div>
        ...
        {step === 0 && (
          <p className="text-center text-sm text-muted-foreground mt-5">
            Déjà un compte ?{" "}
            <Link href="/login" className="text-emerald-400 hover:underline">Se connecter</Link>
          </p>
        )}
      </div>
    </div>
